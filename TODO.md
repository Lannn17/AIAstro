...
- 点光源(PointLight): 从上方打光，制造高光
- 聚光灯(SpotLight): 模拟台灯效果，增强阴影
- 环境贴图(EnvironmentMap): 反射效果，水晶感必备
3. 物理掷骰子模拟
text
真实掷骰子流程:
1. 骰子从手中抛出（初始力 + 旋转扭矩）
...
5. 缓慢停止（阻尼效果）
6. 最终面朝上展示（相机聚焦 + 高亮）
推荐使用 Cannon.js 或 Rapier 做物理引擎，配合 Three.js 渲染。
...
- 星座骰: ♈♉♊♋♌♍♎♏♐♑♒♓
- 宫位骰: 1-12宫
符号应该用 Canvas动态生成纹理 或 预制高清贴图，烫金/浮雕效果。
...
🔵 星座骰 — 深蓝色/靛蓝水晶
🟢 宫位骰 — 深紫色/翡翠色水晶
每面有金色浮雕符号，被投掷时在暗色桌面上翻滚弹跳，最终停下时发出微光，相机缓缓推近展示结果。
