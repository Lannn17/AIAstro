...
### 前端 API 调用：发送 X-Region 头

所有发往 `/api/*` 的 fetch 调用新增请求头（不影响直连高德/Nominatim 的地理编码调用）：

```js
headers: {
  'Content-Type': 'application/json',
  'X-Region': region,   // 从 useRegion() 获取
}
```
...
- `frontend/src/contexts/AuthContext.jsx`（仅 auth 接口，可跳过）
- `frontend/src/hooks/useInterpret.js`
- `frontend/src/pages/Analytics.jsx`

> `AuthContext.jsx` 的 auth 接口无需 region，可跳过。
