...
# 索引优先级: local_index → demo_index → faiss_index
_BASE = pathlib.Path(__file__).parent.parent / "data"
_LOCAL_DIR = _BASE / "local_index"
_DEMO_DIR  = _BASE / "demo_index"
_FULL_DIR  = _BASE / "faiss_index"
...
def _pick_index_dir():
    for d in [_LOCAL_DIR, _DEMO_DIR, _FULL_DIR]:
        if (d / "index.faiss").exists():
            return d
    raise RuntimeError(
        "找不到 FAISS 索引文件。请先运行 build_local_index.py / build_demo_index.py / build_index.py。"
    )
